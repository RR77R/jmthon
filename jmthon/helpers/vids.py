...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            jmthonresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            jmthonfile = await event.client.download_media(jmthonresponse, "./temp")
            return await unzip(jmthonfile)
