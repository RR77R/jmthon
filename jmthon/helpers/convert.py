...
async def convert_to_image(event, client):
    jmthon = await event.get_reply_message()
    if not (
            jmthon.gif
            or jmthon.audio
            or jmthon.voice
            or jmthon.video
            or jmthon.video_note
            or jmthon.photo
            or jmthon.sticker
            or jmthon.media
    ):
        await eor(event, "**⌔∮ هذه الصيغه غير مدعومة**")
        return
    else:
        try:
            c_time = time.time()
            downloaded_file_name = await event.client.download_media(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, event, c_time, "جـار التنزيل انتـظر قليلا")
                ),
            )
            ...
            await eor(event,
                "**⌔∮ تم بنجاح تنزيل ** `{}`.".format(downloaded_file_name)
            )
    if not os.path.exists(downloaded_file_name):
        await eor(event, "التنزيل غير ناجح :(")
        return
    if jmthon and jmthon.photo:
        jmthon_final = downloaded_file_name
    elif jmthon.sticker and jmthon.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        jmthon_final = image_name20
    elif jmthon.sticker and jmthon.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(image_new_path):
            await eor(event, "**- عذرا لقد حصل خطأ ما**")
            return
        jmthon_final = image_new_path
    elif jmthon.audio:
        ...
        if not os.path.exists(imgpath):
            await eor(event, "**- عذرا لقد حصل خطأ ما**")
            return
        jmthon_final = imgpath
    elif jmthon.gif or jmthon.video or jmthon.video_note:
...
def tgs_to_gif(sticker_path: str, quality: int = 256) -> str:                  
    semx = os.path.join(dwlpath, "jmthontgs.gif")
