...
from jmthon.utils.extras import delete_jmthon as eod
...
async def song_search(event, query, max_results, details=False):
    try:
        results = json.loads(JMTHON_YTS(query, max_results=max_results).to_json())
    except KeyError:
        return await eod(event, "**⪼ يتعذر الحصول على معلومات ذات صلة بالبحث**")
    x = ""
